...
class TxGNN:
    ...
    def pretrain(self, n_epoch = 1, learning_rate = 1e-3, batch_size = 1024, train_print_per_n = 20, sweep_wandb = None):
        ...
        # Build flat edge list for batching across all etypes
        all_edges = []  # list of (etype, src_idx, dst_idx)
        ...
        for epoch in range(n_epoch):
            ...
            for step, batch in enumerate(dataloader):
                # Build pos_g from this batch of edge indices
                etype_edges = {}
                for idx in batch:
                    etype, src, dst = all_edges[idx]
                    if etype not in etype_edges:
                        etype_edges[etype] = ([], [])
                    etype_edges[etype][0].append(src)
                    etype_edges[etype][1].append(dst)

                pos_g = HeteroData()
                for nt in self.G.node_types:
                    pos_g[nt].num_nodes = self.G[nt].num_nodes
                for etype, (srcs, dsts) in etype_edges.items():
                    pos_g[etype].edge_index = torch.stack([torch.tensor(srcs, dtype=torch.long),
                                                           torch.tensor(dsts, dtype=torch.long)])

                neg_g = neg_sampler(pos_g)

                pos_g = pos_g.to(self.device)
                neg_g = neg_g.to(self.device)

                pred_score_pos, pred_score_neg, pos_score, neg_score = self.model.forward_minibatch(pos_g, neg_g, self.G.to(self.device), mode = 'train', pretrain_mode = True)
    ...
    def predict(self, df):
        ...
        for ntype in g.node_types:
            g_eval[ntype].num_nodes = g[ntype].num_nodes
        for etype, (src, dst) in out.items():
            g_eval[etype].edge_index = torch.stack([src, dst])
    ...
    def retrieve_sim_diseases(self, relation, k = 5, path = None):
        ...
        for etype in etypes:
            ...
            if etype not in self.G.edge_types:
                out_degrees[etype] = (torch.zeros(0, dtype=torch.long),)
                in_degrees[etype] = (torch.zeros(0, dtype=torch.long),)
                continue
    ...
    def train_graphmask(self, relation = 'indication',
        ...
        self.G = self.G.to(self.device)
        total_edges = sum(self.G[et].num_edges for et in self.G.edge_types)
            ...
            for epoch in range(epochs_per_layer):
                ...
                if self.weight_bias_track == 'True':
                    self.wandb.log({'divergence': g_moving_average.get_value(),
                              ...
                              '%masked_L1': num_masked[0]/total_edges,
                              '%masked_L2': num_masked[1]/total_edges})
    ...
    def retrieve_save_gates(self, path):
        ...
        G = self.G.to('cpu')
        for etypes in G.edge_types:
            etype = etypes[1]
            src, dst = etypes[0], etypes[2]

            df_temp = pd.DataFrame()
            df_temp['x_idx'] = G[etypes].edge_index[0].numpy()
            df_temp['y_idx'] = G[etypes].edge_index[1].numpy()

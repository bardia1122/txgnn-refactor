...
def construct_negative_graph_each_etype(graph, k, etype, method, weights, device):
    utype, _, vtype = etype
    src = graph[etype].edge_index[0]
    dst = graph[etype].edge_index[1]
...
def construct_negative_graph(graph, k, device):
    ...
    for nt in graph.node_types:
        result[nt].num_nodes = graph[nt].num_nodes
    for etype, (neg_src, neg_dst) in out.items():
        result[etype].edge_index = torch.stack([neg_src, neg_dst])
    return result
    ...
    def __init__(self, g, k, method):
        if method == 'multinomial_dst':
            ...
            for etype in g.edge_types:
                ...
                in_deg = degree(g[etype].edge_index[1], num_nodes=g[dst_type].num_nodes)
                self.weights[etype] = in_deg.float() ** 0.75
        elif method == 'fix_dst':
            self.weights = {}
            for etype in g.edge_types:
                src_type, rel, dst_type = etype
                in_deg = degree(g[etype].edge_index[1], num_nodes=g[dst_type].num_nodes)
                self.weights[etype] = (in_deg > 0).float()
        self.k = k

    def __call__(self, batch):
        result = HeteroData()
        for nt in batch.node_types:
            result[nt].num_nodes = batch[nt].num_nodes
        for et in batch.edge_types:
            if et not in self.weights:
                continue
            src = batch[et].edge_index[0]
            src_repeated = src.repeat_interleave(self.k)
            dst = self.weights[et].multinomial(len(src_repeated), replacement=True)
            result[et].edge_index = torch.stack([src_repeated, dst])
        return result
...
class Full_Graph_NegSampler:
    def __init__(self, g, k, method, device):
        if method == 'multinomial_src':
            ...
            for etype in g.edge_types:
                src_type, rel, dst_type = etype
                out_deg = degree(g[etype].edge_index[0], num_nodes=g[src_type].num_nodes)
                self.weights[etype] = out_deg.float() ** 0.75
            ...
            for etype in g.edge_types:
                src_type, rel, dst_type = etype
                out_deg = degree(g[etype].edge_index[0], num_nodes=g[src_type].num_nodes)
                self.weights[etype] = (out_deg > 0).float()
        else:
            self.weights = {}
...
def evaluate_graph_construct(df_valid, g, neg_sampler, k, device):
    ...
    for nt in g.node_types:
        g_valid[nt].num_nodes = g[nt].num_nodes
    for etype, (src, dst) in out.items():
        g_valid[etype].edge_index = torch.stack([src, dst])
...
def evaluate_gnnexplainer(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, penalty_scaling, device, mode = 'validation', weight_bias_track = False, wandb = None):
    ...
    with torch.no_grad():
        ...
        print("Running epoch {0:n} of GNNExplainer training. Mean divergence={1:.4f}, mean penalty={2:.4f}, bce_update={3:.4f}, bce_original={4:.4f}, num_masked_l1={5:.4f}, num_masked_l2={6:.4f}".format(
            ...
            loss_pred,
            loss_pred_ori,
            num_masked[0]/total_edges,
            num_masked[1]/total_edges)
        )
...
def evaluate_graphmask(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, allowance, penalty_scaling, device, mode = 'validation', weight_bias_track = False, wandb = None, no_base = False):
    ...
    with torch.no_grad():
        ...
        if mode == 'testing':
            ...
            test_metrics['test %masked_L1'] = num_masked[0]/total_edges
            test_metrics['test %masked_L2'] = num_masked[1]/total_edges
        ...
        if weight_bias_track:
            wandb.log({mode + ' divergence': float(loss.mean().item()),
                       ...
                       mode + ' %masked_L1': num_masked[0]/total_edges,
                       mode + ' %masked_L2': num_masked[1]/total_edges})
...
def evaluate_mb(model, g_pos, g_neg, G, dd_etypes, device, return_embed = False, mode = 'valid'):
    model.eval()
    pred_score_pos, pred_score_neg, pos_score, neg_score = model.forward_minibatch(g_pos.to(device), g_neg.to(device), G.to(device), mode = mode, pretrain_mode = False)
...
def _get_successors(G, node, etype_str):
    """Helper: get successor nodes for a given node via a relation name string."""
    matching = [et for et in G.edge_types if et[1] == etype_str]
    if not matching:
        return torch.tensor([], dtype=torch.long)
    et = matching[0]
    ei = G[et].edge_index
    node_val = node.item() if hasattr(node, 'item') else int(node)
    mask = ei[0] == node_val
    return ei[1, mask]
...
def obtain_protein_random_walk_profile(disease, num_walks, path_len, g, disease_etypes, disease_nodes, walk_mode):
    random_walks = []
    num_nodes = g['gene/protein'].num_nodes
    for _ in range(num_walks):
        successor = _get_successors(g, disease, 'rev_disease_protein')
        if len(successor) > 0:
            current = choice(successor.tolist())
            current = torch.tensor(current)
        else:
            continue
        path = [current.item()]
        for path_idx in range(path_len):
            successor = _get_successors(g, current.item(), 'protein_protein')
            if len(successor) > 0:
                current = torch.tensor(choice(successor.tolist()))
...
def obtain_disease_profile_blocks(G, disease, disease_etypes, disease_nodes):
    ...
    for idx, disease_etype in enumerate(disease_etypes):
        # Map relation name string to canonical etype tuple
        matching = [et for et in G.edge_types if et[1] == disease_etype]
        if matching:
            et = matching[0]
            ei = G[et].edge_index
            node_val = disease.item() if hasattr(disease, 'item') else int(disease)
            mask = ei[0] == node_val
            nodes = ei[1, mask]
        else:
            nodes = torch.tensor([], dtype=torch.long)
        num_nodes = G[disease_nodes[idx]].num_nodes
        node_profile = torch.zeros((num_nodes,))
        if len(nodes) > 0:
...
def create_pyg_graph(df_train, df):
    """Create a PyG HeteroData graph from training dataframe."""
    unique_graph = df_train[['x_type', 'relation', 'y_type']].drop_duplicates()
    data = HeteroData()

    edge_dict = {}
    for i, row in enumerate(unique_graph.values):
        x_type, relation, y_type = row
        o = df_train[df_train.relation == relation][['x_idx', 'y_idx']].values.T
        src = torch.tensor(o[0].astype(int), dtype=torch.long)
        dst = torch.tensor(o[1].astype(int), dtype=torch.long)
        data[x_type, relation, y_type].edge_index = torch.stack([src, dst])
        if relation not in edge_dict:
            edge_dict[relation] = len(edge_dict)

    # Compute num_nodes per type from max index in full df
    ...
    for ntype, max_idx in output.items():
        data[ntype].num_nodes = int(max_idx) + 1

    # Assign edge type ids
    for et in data.edge_types:
        src_type, rel, dst_type = et
        n_edges = data[et].num_edges
        if rel in edge_dict:
            data[et].edge_id = torch.full((n_edges,), edge_dict[rel], dtype=torch.long)
...
def disease_centric_evaluation(df, df_train, df_valid, df_test, data_path, G, model, device, disease_ids = None, relation = None, weight_bias_track = False, wandb = None, show_plot = False, verbose = False, return_raw = False, simulate_random = True, only_prediction = False):
    ...
    def get_scores_disease(rel, disease_ids):
        ...
        drug_nodes = torch.arange(G['drug'].num_nodes).numpy()
        ...
        for disease_id in tqdm(disease_ids):
            ...
            g_eval = HeteroData()
            for nt in G.node_types:
                g_eval[nt].num_nodes = G[nt].num_nodes
            g_eval[('disease', rel, 'drug')].edge_index = torch.stack([src, dst])
            g_eval = g_eval.to(device)

            model.eval()
            _, pred_score_rel, _, pred_score = model(G, g_eval, g_eval, pretrain_mode = False, mode = 'test')
            pred = pred_score_rel[('disease', rel, 'drug')].reshape(-1,).detach().cpu().numpy()
            edge_dst = g_eval[('disease', rel, 'drug')].edge_index[1].detach().cpu().numpy()
            lab = {idx2id_drug[i]: labels[i] for i in edge_dst}
            preds_contra[idx2id_disease[disease_id]] = {idx2id_drug[i]: pred[idx] for idx, i in enumerate(edge_dst)}
            labels_contra[idx2id_disease[disease_id]] = lab
            ids_contra[idx2id_disease[disease_id]] = edge_dst
...
def remove_random_edges(hetero_graph, K):
    ...
    for etype in hetero_graph.edge_types:
        num_existing_edges = hetero_graph[etype].num_edges
        ...
        edge_ids = torch.tensor(edges_to_remove, dtype=torch.int64)

        mask = torch.ones(num_existing_edges, dtype=torch.bool)
        mask[edge_ids] = False
        hetero_graph[etype].edge_index = hetero_graph[etype].edge_index[:, mask]
...
def remove_relation_type(hetero_graph, relation_type):
    ...
    etypes_all = [et[1] for et in hetero_graph.edge_types]
    if relation_type not in etypes_all:
    ...
    etypes_to_remove = [etype for etype in hetero_graph.edge_types if etype[1] == relation_type]
    etypes_to_remove += [etype for etype in hetero_graph.edge_types if etype[1] == 'rev_' + relation_type]
    ...
    for etype in etypes_to_remove:
        num_edges = hetero_graph[etype].num_edges
...
def add_random_edges(hetero_graph, K):
    ...
    for etype in hetero_graph.edge_types:
        ...
        num_src_nodes = hetero_graph[src_type].num_nodes
        num_dst_nodes = hetero_graph[dst_type].num_nodes
        num_existing_edges = hetero_graph[etype].num_edges
        ...
        src_tensor = torch.tensor(src_random_nodes, dtype=torch.int64)
        dst_tensor = torch.tensor(dst_random_nodes, dtype=torch.int64)

        new_edges = torch.stack([src_tensor, dst_tensor])
        hetero_graph[etype].edge_index = torch.cat([hetero_graph[etype].edge_index, new_edges], dim=1)
...
def randomize_edges(hetero_graph):
    ...
    for etype in hetero_graph.edge_types:
        ...
        hetero_graph[etype].edge_index = torch.stack([src_tensor, dst_tensor])

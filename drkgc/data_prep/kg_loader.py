...
from drkgc.config import DEFAULT_KG_FOLDER, DISEASE_FILES_DIR, PYG_ROOT
...
def ensure_node_csv(data_folder: Path = DEFAULT_KG_FOLDER) -> Path:
    """Make sure `node.csv` exists in the tab-separated form `DataSplitter` wants.

    `DataSplitter.load_kg` reads `node.csv` with `sep='\\t'` (that is how TxGNN
    ships it on Dataverse), but the official PrimeKG release provides the same
    table as a **comma-separated `nodes.csv`**. If only the latter is present we
    transcode it rather than downloading a second copy.
    """
    data_folder = Path(data_folder)
    node_csv = data_folder / "node.csv"
    if node_csv.exists():
        return node_csv

    nodes_csv = data_folder / "nodes.csv"
    if nodes_csv.exists():
        print(f"node.csv missing - transcoding {nodes_csv.name} to tab-separated node.csv ...")
        pd.read_csv(nodes_csv, low_memory=False).to_csv(node_csv, sep="\t", index=False)
        return node_csv

    ensure_raw_kg(data_folder, required=("node.csv",))
    return node_csv


def area_kg_folder(data_folder: Path, area: str) -> Path:
    """Where `preprocess_kg` puts a disease-area KG (`utils.py:86-97`)."""
    return Path(data_folder) / f"{area}_kg"


def load_area_kg_directed(
    data_folder: Path = DEFAULT_KG_FOLDER,
    area: str = "cardiovascular",
    seed: int = 42,
) -> pd.DataFrame:
    """Directed KG for one disease area, with TxGNN's `split` column.

    Generated by `preprocess_kg(data_folder, split=area)`, which uses the
    Disease Ontology (`DataSplitter.get_test_kg_for_disease`) to mark, as
    `split == 'test'`, every drug-disease edge touching that area plus a random
    sample of edges from the area's 2-hop neighbourhood. Everything else is
    `split == 'train'`.

    Note that TxGNN's sampling of those neighbourhood edges calls
    `np.random.choice` **without seeding** (`datasplit.py:109`), so we seed the
    global numpy RNG first to make the generated KG reproducible.
    """
    utils = txgnn_utils()
    data_folder = Path(data_folder)
    kg_directed = area_kg_folder(data_folder, area) / "kg_directed.csv"

    if not kg_directed.exists():
        ensure_raw_kg(data_folder, required=("kg.csv", "edges.csv"))
        ensure_node_csv(data_folder)
        print(f"Generating the {area!r} disease-area KG (ontology + 2-hop sampling) ...")
        import numpy as np

        np.random.seed(seed)  # datasplit.py samples unseeded; make it reproducible
        utils.preprocess_kg(str(data_folder), split=area)
    ...
    if "split" not in df.columns:
        raise ValueError(
            f"{kg_directed} has no 'split' column - it does not look like a "
            "disease-area KG."
        )
    counts = df.split.value_counts().to_dict()
    print(f"Area KG {area!r}: {len(df):,} directed edges, split counts {counts}")
    return df
...
def load_kg_directed(
    ...
    """Return the directed PrimeKG dataframe (`kg_directed.csv`).

    With `area` set, returns that disease area's KG instead (see
    :func:`load_area_kg_directed`), which additionally carries a `split` column.
    ...
    * `x_idx` / `y_idx` are contiguous per-node-type integer indices, stored as
    ...
    """
    if area:
        return load_area_kg_directed(data_folder, area, seed)
...
def area_disease_indices(
    df: pd.DataFrame,
    area: str,
    disease_files_dir: Path = DISEASE_FILES_DIR,
) -> set:
    """The `disease` node indices belonging to one disease area.

    Reproduces `process_disease_area_split` (`utils.py:1057-1083`): build
    `raw disease id -> x_idx` from the KG, extend it so each part of a merged id
    (`'8450_15304'`) also maps to that node, then look up every `node_id` listed
    in `data/disease_files/<area>.csv`.
    """
    utils = txgnn_utils()
    path = Path(disease_files_dir) / f"{area}.csv"
    if not path.exists():
        raise FileNotFoundError(f"no disease-area file at {path}")
    disease_list = pd.read_csv(path)
    ...
    for raw_id, idx in id2idx.items():
        try:
            if "_" in str(raw_id):
                for part in str(raw_id).split("_"):
                    extra[str(float(part))] = idx
            else:
                extra[str(float(raw_id))] = idx
        except (TypeError, ValueError):
            continue
    id2idx.update(extra)

    mapped = disease_list.node_id.apply(lambda x: utils.map_node_id_2_idx(x, id2idx))
    found = {int(i) for i in mapped if i != "null"}
    missing = int((mapped == "null").sum())
    print(
        f"  disease area {area!r}: {len(found):,} of {len(disease_list):,} listed "
        f"diseases resolved to KG indices ({missing} unmatched)"
    )
    return found
